...
- После загрузки полной вакансии deterministic policy исключает экосистемы Сбера/Яндекса и betting-сектор до чтения базы знаний, обращения к ИИ и application preflight.
...
Секция `[search]` содержит `excluded_employer_terms` и `excluded_industry_terms`. Заданные значения только расширяют обязательные встроенные исключения: Сбер/Sber и Яндекс/Yandex распознаются в именах работодателей с дочерними суффиксами и разделителями, а букмекерские термины и явные бренды — в работодателе, названии и описании. Общий substring `bet` намеренно запрещён как слишком широкий. Совпадение записывается как terminal `ineligible` с причиной `excluded_employer` или `excluded_betting`; вакансия не передаётся модели и не проходит preflight.
