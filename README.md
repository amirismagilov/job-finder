...
3. На странице настроек расширения нажмите «Создать новый», затем «Сохранить и включить».
4. Выполните `.venv/bin/job-finder bridge pair` и вставьте тот же секрет в скрытый ввод.
5. Расширение само проверит готовность content script через ограниченный handshake; если старая вкладка была открыта до установки, она будет один раз перезагружена без кликов по сайту.
